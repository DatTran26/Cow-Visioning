const Camera = (() => {
    ...
    async function captureAndSave() {
        ...
        // If metadata is not filled, keep preview but skip upload
        if (!cowId || !behavior || !barnArea) {
            cameraStatus.textContent = 'Đã chụp ảnh xem trước. Điền Thiết lập để lưu ảnh lên hệ thống.';
            cameraStatus.className = 'status-msg info';
            if (isOn) shutterBtn.disabled = false;
            return;
        }
